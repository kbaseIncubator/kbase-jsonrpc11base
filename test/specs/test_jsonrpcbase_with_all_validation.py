...
from jsonrpc11base.exceptions import DuplicateMethodName
import pytest
import os


class InvalidServerCode(APIError):
    code = -32000
...
def service():
    ...
    def hello_invalid_result(options):
        return 123
    ...
    service.add(hello, name="hello_no_result_validation")
    service.add(hello, name="hello_absent_result_validation")
    service.add(hello_invalid_result)
...
def test_no_params_validation(service):
    """
    When a method expects args, but none are provided,
    should return error.
    """
...
def test_no_result_validation(service):
    """
    Test valid jsonrpc no argument calls.
    """
    res = service.call_py({
        "version": "1.1",
        "method": "hello_no_result_validation"
    })
    assert res['error']['message'] == 'Invalid params'
    assert res['error']['code'] == -32602
    assert res['error']['error']['message'] == ('Validation is enabled, '
                                                'but no result validator was provided')


def test_invalid_result(service):
    """
    Test invalid result
    """
    res = service.call_py({
        'version': '1.1',
        'method': 'hello_invalid_result'
    })
    assert res['error']['message'] == 'Invalid result'
...
def test_incorrect_absent_result_validation(service):
    """
    Test that a method which returns yet has an absent result schema,
    returns an Invalid params error
    """
    res = service.call_py({
        "version": "1.1",
        "method": "hello_absent_result_validation"
    })
    assert res['error']['message'] == 'Invalid result'
    assert res['error']['code'] == -32002
    assert res['error']['error']['message'] == ('The method is specified to not return a result, '
...
def test_duplicate_method_name_err(service):
    ...
    """
    with pytest.raises(DuplicateMethodName) as excinfo:
